...
public class Rebarbaro implements CXPlayer {
	...
	public int selectColumn(CXBoard B) {
		...
		//for (int col : L) {
			...
			int score = minimax(B, depth, Integer.MIN_VALUE, Integer.MAX_VALUE, true); //minimax
			//System.err.print("score: " + score); //debug //se il punteggio E' migliore di quello attuale
				bestScore = score; //lo aggiorno //e aggiorno la colonna migliore
	...
	public int minimax(CXBoard B, int depth, int alpha, int beta, boolean maximizingPlayer) {
		...
		if(state == myWin) return 1000;
		if(state == yourWin) return -1000;

		if (depth == 0 || state == CXGameState.DRAW) {
			return evaluationFunction(B, maximizingPlayer);
		}
		...
		if (maximizingPlayer) {
			...
			int maxScore = Integer.MIN_VALUE;
			for (int col : L) {
				B.markColumn(col); //marco la mossa
	...
	public int evaluationFunction(CXBoard board, boolean maximizingPlayer) {
		int myPieces = 0;
		int myThrees = 0;
		int myTwos = 0;
		int myVerticalWins = 0;
		...
		int score = (myThrees * 10) + (myTwos * 5) + (2 * myVerticalWins);
		return score;
	}
