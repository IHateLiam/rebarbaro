...
public class Rebarbaro implements CXPlayer {
	...
	public int selectColumn(CXBoard board) {
		...
		for (int col : availableColumns) {
			try {
				...
				score = minimax(board, depth, col, Integer.MIN_VALUE, Integer.MAX_VALUE, false, myComboListCopy, advCombosCopy); //minimax
	...
	public float minimax(CXBoard board, int depth, int firstMove, float alpha, float beta, boolean maximizingPlayer, LinkedList<Combo> originalRebarbaroCombos, 
						LinkedList<Combo> originalAdvCombos) {
		//tempo
		...
		if (maximizingPlayer) {
			...
			for (int col : availableColumns) {
				...
				score = minimax(board, depth - 1, col, alpha, beta, false, rebarbaroCombos, advCombos);
			...
			for (int col : availableColumns) {
				...
				score = minimax(board, depth - 1, col, alpha, beta, true, rebarbaroCombos, advCombos);
